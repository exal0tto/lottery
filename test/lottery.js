...
describe('Lottery', () => {
  ...
  let snapshot;
  let lottery, partnerLottery, playerLottery;
  ...
  before(async () => {
    ...
    player = await signers[2].getAddress();
    ...
    partnerLottery = await lottery.connect(signers[1]);
    playerLottery = await lottery.connect(signers[2]);
  ...
  describe('getTicketPrice', () => {
    ...
    it('6 numbers', async() => {
      const price = (await lottery.getBaseTicketPrice()).toBigInt();
  ...
  describe('value distribution', () => {
    ...
    it('funding', async () => {
      ...
      const funds = 123456n;
      await lottery.fund({value: funds});
  ...
  describe('getRoundData', () => {
    ...
    it('1 round', async () => {
      ...
      const prize = value * 188n / 1000n;
      const stash = value - prize * 5n;
      const block = await helpers.time.latestBlock();
      await draw();
      await expect(lottery.getRoundData(0)).to.be.reverted;
  ...
  describe('draw', () => {
    it('next draw time', async () => {
      const nextDrawTime = new Date(1000 * await lottery.getNextDrawTime());
    ...
    it('cannot draw', async () => {
      ...
      await expect(lottery.draw(subscriptionId, process.env.CHAINLINK_VRF_KEY_HASH)).to.be.reverted;
    });
    ...
    it('skip a draw', async () => {
      ...
      expect(nextDrawTime).to.be.above(await helpers.time.latest());
  ...
  describe('prizes', () => {
    it('withdraw', async () => {
      ...
      expect(ticket.player).to.equal(player);
      expect(ticket.prize).to.equal(prize);
      expect(ticket.withdrawn).to.equal(false);
      expect(await lottery.provider.getBalance(lottery.address)).to.equal(value);
      await lottery.withdrawPrize(1);
      expect(await lottery.provider.getBalance(lottery.address)).to.equal(value - prize);
      ticket = await lottery.getTicketPrize(1);
      expect(ticket.player).to.equal(player);
      expect(ticket.prize).to.equal(prize);
      expect(ticket.withdrawn).to.equal(true);
    });
    ...
    it('two withdrawals', async () => {
      ...
      expect(ticket2.player).to.equal(player);
      expect(ticket2.prize).to.equal(prize);
      expect(ticket2.withdrawn).to.equal(false);
      await lottery.withdrawPrize(2);
      ticket2 = await lottery.getTicketPrize(2);
      expect(ticket2.player).to.equal(player);
      expect(ticket2.prize).to.equal(prize);
      expect(ticket2.withdrawn).to.equal(true);
      expect(await lottery.provider.getBalance(lottery.address)).to.equal(value - prize * 2n);
      await expect(lottery.withdrawPrize(1)).to.be.reverted;
  ...
  describe('higher order tickets', () => {
    it('7 numbers', async () => {
      ...
      expect(ticket.withdrawn).to.equal(false);
      await lottery.withdrawPrize(2);
      expect(await lottery.provider.getBalance(lottery.address)).to.equal(
          value - ticket.prize.toBigInt());
